...
uses
  ...
  uLists, uImageFormats, uImageLoader, uPerlinNoise;
...
type
  ...
  public
    ...
    procedure GenerateNoise(aData: pointer; aSize: integer);
...
{$ENDIF}
  ...
  //Fill bitmap with noise image
  GenerateNoise(GS8Image.Data, 256);
...
procedure TDemo.FillImage(aData: pointer; aFormat: cardinal; aSize: integer);
var ps: byte;
    i,j, count: integer;
begin
...
procedure TDemo.GenerateNoise(aData: pointer; aSize: integer);
var noise: TByteArray2d;
  i,j: integer;
begin
  GenNoise(noise, aSize);
  for i:=0 to 255 do for j:=0 to 255 do
    PByteArray(aData)[i*256+j]:=noise[i,j];
end;
