...
begin
  ...
  with GLViewer1.Context do
    if (MaxMajorVersion<4) or (MaxMinorVersion<2) then begin
      GL1xRender:=true; ver.GAPI:=avGL; ver.Version:=130;
    end else begin
      ver.GAPI:=avGL; ver.Version:=420;
    end; Render:=nil;
...
//  assert(Test,'Vector Math Test Failed');
  ...
  if not GL1xRender then begin
   ...
   {$ENDIF}
    ...
    Shader1.LinkShader;
    if Shader1.Error then begin showmessage(Shader1.Log); Halt(0); end;
  ...
  end else Shader1:=nil;

  //Making MVP Matrix
  proj:=TMatrix.PerspectiveMatrix(60,GLViewer1.Width/GLViewer1.Height,0.1,100);
  cameraPos := TVector.Make(0,0,-5);
  view:=TMatrix.LookAtMatrix(cameraPos, VecNull, vecY);
  model.SetIdentity;

  Quad:=MakeQuad; Quad.Shader:=Shader1;
  Sphere:=MakeSphere; Sphere.Shader:=Shader1;
  Box:=MakeBox; Box.Shader:=Shader1;
  Teapod := MakeTeapod; Teapod.Shader := Shader1;
...
procedure TForm2.GL1xDebugRender(const Mesh: TGLVertexObject; const Model: TMatrix);
var i: integer;
    p,n,t: pointer;
    attr: TGLAttribObject;
const CGLFaceType: array [ftPoints .. ftQuads] of cardinal =
  (
...
{ //Со временем здесь будет полноценный ренедер сцены
  ...
  mvp:=Model*View*Proj;
  if not GL1xRender then begin
    Shader1.Apply; Shader1.SetUniform('MVP',mvp.Matrix4); Shader1.UnApply;
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
//    TGLStaticRender.RenderVertexObject(box,slUseActiveShader, spUseActiveShaderFirst);
    Box.RenderVO;
    Quad.RenderVO; Sphere.RenderVO; Teapod.RenderVO();
  end else begin
